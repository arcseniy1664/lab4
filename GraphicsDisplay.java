package bsu.rfe.java.group6.lab4.Serba.var6A;
...
// Перо для рисования графика
        float[] dash = {10,7,3,7,3,7,3,7,10,7,3,7,10,3};
...
// Отобразить график
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            int radius = Math.min(width, height) / 80;
            // Рисуем окружность
            canvas.drawOval((int)center.x - radius, (int)center.y - radius, radius * 2, radius * 2);

            // Рисуем крест
            canvas.drawLine((int)center.x - radius,(int) center.y, (int)center.x + radius, (int)center.y); // Горизонтальная линия
            canvas.drawLine((int)center.x, (int)center.y - radius, (int)center.x,(int) center.y + radius); // Вертикальная линия
        }
    }
